...
- No package changes — synchronized with native v0.3.10 (`EXTENSION_CONSTRAINT` now `^0.3.10`). The native release fixes two reliability bugs: channel-scoped AMQP errors (delay keep-alive redeclares) no longer tear down the whole connection, and permanent recovery failures (authentication, protocol) fail the pool instead of retrying forever.
